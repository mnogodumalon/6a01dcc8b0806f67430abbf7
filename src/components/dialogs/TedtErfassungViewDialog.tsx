...
interface TedtErfassungViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: TedtErfassung | null;
  onEdit: (record: TedtErfassung) => void;
}

export function TedtErfassungViewDialog({ open, onClose, record, onEdit }: TedtErfassungViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Tedt Erfassung anzeigen</DialogTitle>
        </DialogHeader>
