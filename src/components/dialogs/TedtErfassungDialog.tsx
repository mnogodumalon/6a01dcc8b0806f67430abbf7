...
interface TedtErfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: TedtErfassung['fields']) => Promise<void>;
  defaultValues?: TedtErfassung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function TedtErfassungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: TedtErfassungDialogProps) {
  const [fields, setFields] = useState<Partial<TedtErfassung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'tedt_erfassung');
      await onSubmit(clean as TedtErfassung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel\n  "beschreibung": string | null, // Beschreibung\n  "datum": string | null, // YYYY-MM-DD\n  "status": LookupValue | null, // Status (select one key: "offen" | "in_bearbeitung" | "abgeschlossen") mapping: offen=Offen, in_bearbeitung=In Bearbeitung, abgeschlossen=Abgeschlossen\n  "anmerkungen": string | null, // Anmerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<TedtErfassung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Tedt Erfassung bearbeiten' : 'Tedt Erfassung hinzufügen';
