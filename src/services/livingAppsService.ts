...
import type { TedtErfassung, CreateTedtErfassung } from '@/types/app';
...
export class LivingAppsService {
  // --- TEDT_ERFASSUNG ---
  static async getTedtErfassung(): Promise<TedtErfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEDT_ERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as TedtErfassung[];
    return enrichLookupFields(records, 'tedt_erfassung');
  }
  static async getTedtErfassungEntry(id: string): Promise<TedtErfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEDT_ERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as TedtErfassung;
    return enrichLookupFields([record], 'tedt_erfassung')[0];
  }
  static async createTedtErfassungEntry(fields: CreateTedtErfassung) {
    return callApi('POST', `/apps/${APP_IDS.TEDT_ERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'tedt_erfassung') });
  }
  static async updateTedtErfassungEntry(id: string, fields: Partial<CreateTedtErfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.TEDT_ERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'tedt_erfassung') });
  }
  static async deleteTedtErfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TEDT_ERFASSUNG}/records/${id}`);
  }
