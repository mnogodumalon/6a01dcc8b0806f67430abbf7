...
import { TedtErfassungViewDialog } from '@/components/dialogs/TedtErfassungViewDialog';
...
const ENTITY_TABS = [
  { key: 'tedt_erfassung', label: 'Tedt Erfassung', pascal: 'TedtErfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('tedt_erfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'tedt_erfassung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'tedt_erfassung': return (data as any).tedtErfassung as TedtErfassung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'tedt_erfassung': return TEDTERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'tedt_erfassung': return {
        create: (fields: any) => LivingAppsService.createTedtErfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTedtErfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTedtErfassungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'tedt_erfassung' || dialogState?.entity === 'tedt_erfassung') && (
        <TedtErfassungDialog
          open={createEntity === 'tedt_erfassung' || dialogState?.entity === 'tedt_erfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'tedt_erfassung' ? handleUpdate : (fields: any) => handleCreate('tedt_erfassung', fields)}
          defaultValues={dialogState?.entity === 'tedt_erfassung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['TedtErfassung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['TedtErfassung']}
        />
      )}
      {viewState?.entity === 'tedt_erfassung' && (
        <TedtErfassungViewDialog
          open={viewState?.entity === 'tedt_erfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'tedt_erfassung', record: r }); }}
        />
      )}
