...
export default function TedtErfassungPage() {
  const [records, setRecords] = useState<TedtErfassung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<TedtErfassung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<TedtErfassung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<TedtErfassung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getTedtErfassung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: TedtErfassung['fields']) {
    await LivingAppsService.createTedtErfassungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: TedtErfassung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTedtErfassungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Tedt Erfassung"
      subtitle={`${records.length} Tedt Erfassung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Tedt Erfassung suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Tedt Erfassung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
